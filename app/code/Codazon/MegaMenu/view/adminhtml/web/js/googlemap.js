define(['jquery'], function($) {
	$.widget('codazon.googlemap', {
		options: {
			...
			mapAddress: '',
			markerTitle: '',
			jsSource: '//maps.googleapis.com/maps/api/js?v=3.17&signed_in=true&key=AIzaSyBPhrY0hWJrTFqyf_j4TJhBTe9oYg6ASFs',
		},
		_create: function(){
			...
			require([config.jsSource],function(){
				...
				var mapOptions = {
					zoom: config.mapZoom,
					center: {lat: -1.2674513, lng: 36.8308651}
				};
				var map = null;
				function createMap(){
					var map = new google.maps.Map(self.element.get(0), mapOptions);
					var marker = new google.maps.Marker({
						position: {lat: -1.2674513, lng: 36.8308651},
						map: map,
